        ...
        if health:
            if mm:  # READY
                ...
                if 'STAY' in pos.actions:
                    ...
                    u_default = get_util(state, itr, pos=pos.default())
                    # ORIGINAL CODE:
                    # u_independant = p1 * (STEPCOST + GAMMA*u_success) + (1-p1)*(STEPCOST + GAMMA*u_default)
                    # TASK 2.2:
                    u_independant = p1 * (GAMMA*u_success) + \
                        (1-p1)*(GAMMA*u_default)
                    if penalty == 0:
                        ...
                        #    (1-p1)*(STEPCOST + GAMMA*u_default_d)
                        # TASK 2.2:
                        u_dormant = p1 * (GAMMA*u_success_d) + \
                            (1-p1)*(GAMMA*u_default_d)
                        util = 0.5*(u_independant) + 0.5*(u_dormant)
                    else:
                        # ORIGINAL CODE:
                        # util = 0.5*(u_independant) + 0.5 * (STEPCOST + penalty + GAMMA*u_failure)
                        # TASK 2.2:
                        util = 0.5*(u_independant) + 0.5 * \
                            (penalty + GAMMA*u_failure)
                ...
                if 'STAY' in pos.actions:
                    ...
                    # ORIGINAL CODE:
                    # u_independant = p1 * (STEPCOST + GAMMA*u_success) + (1-p1)*(STEPCOST + GAMMA*u_default)
                    # u_ready = p1*(STEPCOST + GAMMA*u_success_r) + (1-p1)*(STEPCOST + GAMMA*u_default_r)
                    # TASK 2.2:
                    u_independant = p1*(GAMMA*u_success) + \
                        (1-p1)*(GAMMA*u_default)
                    u_ready = p1*(GAMMA*u_success_r) + \
                        (1-p1)*(GAMMA*u_default_r)
                    util = 0.2*(u_ready) + 0.8*(u_independant)
                    u_t1.update({util: 'STAY'})
...
if __name__ == '__main__':
    ...
    n = num_iter(states)
    f = open('./part_2_task2.2_trace.txt', 'w')
    for i in range(1, n):
        trace(i, f)
    f.close()
